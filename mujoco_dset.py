...
class Mujoco_Dset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=-1, randomize=True):
        ...
        print(traj_data['obs'].shape)
        print(traj_limitation)
        if traj_limitation < 0:
            traj_limitation = len(traj_data['obs'])
        obs = traj_data['obs'][:traj_limitation]
        acs = traj_data['acs'][:traj_limitation]

        # BUGFIX: backed out change from https://github.com/openai/baselines/pull/447, but with return y still un-indented
        def flatten(x):
            # x.shape = (E,), or (E, L, D)
            _, size = x[0].shape
